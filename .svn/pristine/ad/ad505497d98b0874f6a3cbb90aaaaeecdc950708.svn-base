...
//                     URLEncoder.encode(entry.getValue(), "UTF-8")));
    ...
    public static String post(String actionUrl, Map<String, String> params,
        ...
        if (BaseApp.DEBUG) {//调试模式  不加密
            LogUtil.d("TokenId" + tokenid);
            conn.setRequestProperty("TokenId", tokenid);
        } else {//发布模式 加密
            if (actionUrl.contains("Chinarcb/UploadFiles") ||
                    actionUrl.contains("file/UploadSingleFile")) {
        ...
        if (null != files) {
            for (Map.Entry<String, File> file : files.entrySet()) {
                System.out.println("key=" + file.getKey() + ",value=" + file.getValue().getAbsolutePath());
                String key = file.getKey().startsWith("FilesMulti") ? "Files" : file.getKey();
                System.out.println("key1=" + key + ",value=" + file.getValue().getAbsolutePath());
                ...
                InputStream is = new FileInputStream(file.getValue().getAbsolutePath());
