        ...
        if (logHeaders) {
            ...
            for (int i = 0, count = headers.size(); i < count; i++) {
                ...
                if (!"Content-Type".equalsIgnoreCase(name) && !"Content-Length".equalsIgnoreCase(name)) {
                    ...
                    if (!BaseApp.DEBUG) {//发布模式
                        String nameValue = new String(Base64.decode(headers.value(i), Base64.DEFAULT), "utf-8");
                        logger.log("请求头:" + name + ":" + nameValue);
                    }
                }
            }
            ...
            if (!logBody || !hasRequestBody) {
                ...
                if (isPlaintext(buffer)) {
                    ...
                    logger.log("上传参数:" + result);
                    if (!BaseApp.DEBUG) {//发布模式
                        String resultDecode = AESUtil.desEncrypt(result);
                        logger.log("上传参数:" + resultDecode);
                    }
                    logger.log("--> END " + request.method()
        ...
        if (logHeaders) {
            ...
            if (!logBody || !HttpHeaders.hasBody(response)) {
                ...
                if (contentLength != 0) {
                    ...
                    logger.log("返回结果:" + result);
                    if (!BaseApp.DEBUG) {//发布模式
                        logger.log("返回结果:" + AESUtil.desEncrypt(result));
                    }
                }
