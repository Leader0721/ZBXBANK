    ...
    public void setImage(final String url, final ImageView view, WXImageQuality quality, final WXImageStrategy strategy) {
        WXSDKManager.getInstance().postOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                LoadLocalImageUtil.getInstance().dispalyFromAssets("weex" + url, view);
                /*if (url.contains("/assets/")) {
                    LoadLocalImageUtil.getInstance().dispalyFromAssets(url.replace("/assets/",""), view);
                    return;
                }
//                if (url.contains("/mipmap/")) {
//                    LoadLocalImageUtil.getInstance().dispalyFromAssets(url.replace("/mipmap/",""), view);
//                    return;
//                }
//                if (url.contains("/drawable/")) {
//                    LoadLocalImageUtil.getInstance().displayFromDrawable(url.replace("/drawable/",""), view);
//                    return;
//                }
